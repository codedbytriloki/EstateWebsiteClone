  ...
  return (
    <div className="absolute top-0 left-0 w-full z-10">
      <div className='Container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>
        ...
        <img onClick={() => setShowMenu(true)} src={assets.menu_icon} alt="" className='md:hidden w-7 cursor-pointe' />
      </div>
      {/* --------------- mobile - menu ------------- */}
      <div className={`md:hidden ${showMenu ? 'fixed w-700' : 'h-0 w-0'}  right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        <div className='flex justify-end p-6 cursor-pointer'>
          <img onClick={() => setShowMenu(false)} src={assets.cross_icon} className="w-4" />
        </div>
